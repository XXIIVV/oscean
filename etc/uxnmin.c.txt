...
typedef struct {
	unsigned char ram[0x10000], dev[0x100];
	Stack wst, rst;
} Uxn;
...
emu_deo(unsigned char addr, unsigned char value)
{
	uxn.dev[addr] = value;
	switch(addr) {
	case 0x18: fputc(uxn.dev[addr], stdout); return;
	case 0x19: fputc(uxn.dev[addr], stderr); return;
	}
}
...
{
	unsigned int a, b, c, x[2], y[2], z[2], step = 0x80000;
