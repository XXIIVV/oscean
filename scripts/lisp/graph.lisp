...
(dom:set-html _activity 
  (concat 
    (wrap 
      (link "calendar" "Calendar" "local calendar sprite_calendar") "li") 
    (wrap 
      (link "journal" "Journal" "local journal sprite_journal") "li") 
    (wrap 
      (link "tracker" "Tracker" "local tracker sprite_tracker") "li")))
; footer
(def __socials 
  (concat 
    ...
    (link "Devine Lu Linvega") " © 06I04 — " 
    (arvelie) 
    (wrap 
      (concat 
        (link "About" "BY-NC-SA 4.0") " " 
        (neralie)) "center")))
(dom:set-html _footer 
  (wrap 
    (concat __socials) "div" "wr"))
(dom:append _terminal 
  (_termhand _termview))
(dom:append _glyph 
  (_path))
(dom:append _info 
  (_glyph _title))
(dom:append _menu 
  (_info _logo _search _activity))
(dom:append _header 
  (_photo _menu))
(dom:append _core 
  (_sidebar _content _portal))
(dom:append dom:body 
  (_terminal _header _core _footer))
`