...
DICTIONARISM
  UNDE : Automatons
  BREF : {{Dictionarism}} is a simple twitter bot that generates {_-isms_} from every of every single word of a dictionary and tweets them.
  LONG
    - Mirrorism
    - Polygonism
    - Neptunism
...
ADVENT V
  UNDE : Automatons
  BREF : {{Advent V}}, is a simple twitter game in the format of the {_Choose Your Own Adventure_}.
  LONG
...
DILUVIUM
  ...
  LONG
    ...
    & {*Update:*} A new version was released on june 15th 2012 including better online accessiblity, animal balancing and minor networking fixes.
    * Gameplay
    ...
    * Multiplayer
    & By typing {*LOCAL*} in the login you will then find yourself battling the computer and possibly discover new animals. But the real fun begins by challenging other human summoners
    & If you wish to start your own arena, type {*HOST RABBIT*} in the splash screen input. This will initiate an arena called "rabbit" and other players will be able to join it by typing {*JOIN RABBIT*} - you don't have to pick rabbit as game name.
    & If you want to create a nameless match, simply type {*HOST*}, without a game name, and other players will be able to join it by typing {*JOIN*} and by typing your IP Address if the other player is on the same local network.
...
INSTRUMENT
  UNDE : Pino
  BREF : The {{Instrument}} is {{Pino}}'s sailing computer.
  TYPE : Diary
  LONG
...
UNRELEASED
  UNDE : Interactive
  BREF : A list of {{Unreleased}} projects, or abandoned projects, indefinite put on hold and unsupported.
  TYPE : Portal
  
BLINDFOLK
  UNDE : Unreleased
  BREF : {{Blindfolk}} was a cryptic multiplayer combat game where players must program their character's fighting style.
  LONG
...
TRIMETRIQUE
  UNDE : Unreleased
  TYPE : Diary
  BREF : {{Trimetrique}} is a light-painting mobile application.

EAKOUT
  UNDE : Unreleased
  TYPE : Diary
  BREF : {{Eakout}} is a two player breakout game for mobile.
...
MILAVREGA
  UNDE : Unreleased
  BREF : {{Milavrega}} was a mobile game happening in someone's lost phone.
  LONG
...
KEYBOARD 468
  ...
  TYPE : Diary
  LONG
    & So you always only need to type using the two rows at the top, maximizing the space and allowing for big large letters. The hidden letters are accessible through the {_alt_} key, alongside symbols and numbers.
    * Privacy Policy
...
NOIRCA
  UNDE : Mobile
  BREF : {{Noirca}} is a minimalist monochromatic camera application for iOS.
  TYPE : Diary
  LONG
...
COLOUR
  UNDE : Artwork
  BREF : The {{Colour}} collection are a series of experiments involving colours.
...
PEARLS
  UNDE : Nereid
  TYPE : Diary
  BREF : Looking into the infinitely veiled.
...
KALEIDOSCOPE
  UNDE : Polygonoscopy
  BREF : The {{Polygonoscopic|Polygonoscopy}} {{Kaleidoscope}} records the intersection of overlapping structures, our reality's z-fighting.
  LONG
...
BRANE
  UNDE : Polygonoscopy
  BREF : {{Branes|Brane}} are digital fabrics.
  LONG
...
NERVOUS
  UNDE : Polygonoscopy
  BREF : {{Nervous}} is a eries of {_Vertexbrates_}.
  TYPE : Diary

LARD SHADER
  UNDE : Artwork
  BREF : This is a 3d vertex shader applied on a 2d image, transforming neoclassical painted figures into plump versions of themselves.
  LONG
...
CYANOSIS FEVER
  UNDE : Unity
  BREF : {{Cyanosis Fever}} is an 3d abstract world of static to explore and to get lost into.
  LONG
...
VOLKENESSEN
  ...
  LONG
    ...
    & The game is available for download for OS X and Windows (64bits), for the non-64bits version, use the mirror on the {{Global Game Jam 2012 site|http://globalgamejam.org/2012/volkenessen}}.
    & You {*need a controller*} to play this game, it simply won't work with a keyboard. It was tested with the XBox controller and some generic Logitech controller. It will be presented at the {{IGDA|http://www.igda.org/montreal/Jan31_DemoNight}} tomorrow evening at the demo night and publicly playable for the first time at the next {{Indies Meetup|https://www.facebook.com/events/182113635223855/}} and {{Toy Company festival|http://toycompany.cc/}} in February.
    * Controls
...
PICO BATTLE
  ...
  LONG
    & {{Renaud Bedard|http://theinstructionlimit.com/}} and I showcased an experimental game at {*Prince of the Arcade*} last year but, between {{Volkenessen}}, {{Diluvium}} and {{Waiting for Horus}}, only got back to release it now. It is simply called Pico Battle and like our other {{Pico}} titled game - this game plays with colours; but this time, in multiplayer.
...
TARPOPHOBIA
  UNDE : Polygonoscopy
  BREF : {{Tarpophobia}} is the fear of seeing yourself fractalized onto reflective surfaces, especially onto eyes.
  TYPE : Diary
...
TIME ALLOY
  UNDE : Polygonoscopy
  BREF : The {{Time Alloy}} is a series of {{Polygonoscopic|Polygonoscopy}} samples, taken from {{The sartre mechanism}}.
  TYPE : Diary
...
SOUNDTRACK
  UNDE : Aliceffekt
  BREF : The {{Soundtrack}} collection contains various scores written for {{Interactive}} projects.
  TYPE : Portal
...
VAST
  UNDE : Physical
  BREF : {{Vast}} is the first book of {{Elodie Lareine|http://noirmirroir.com/}}, a manual of procedural imagery.
  TYPE : Diary
  LONG
...
FILM
  ...
  LONG
    & This album will be updated periodically, shot with the Voightlander {{250 Jaher|Camera}}. A {{black}} & white album is also maintained.
  TYPE : Diary

BLACK
  UNDE : Photography
  BREF : {{Black}} & White film photography diary of the life aboard {{Pino}}.
  LONG
    & This album will be updated periodically, shot with the Voightlander {{250 Jaher|Camera}}.
  TYPE : Diary
...
TRAVEL
  UNDE : Photography
  BREF : Photoblog from {{Travels|Travel}} around the world, I highlighted some of my favourite trips in the list below.
  TYPE : Portal
...
AMERICA
  ...
  LONG
    & Some taken while in a 52 hours train ride during {{Train Jam|http://trainjam.com}} in 2015, from Chicago to San Francisco
  TYPE : Diary
...
SOUTH PACIFIC
  UNDE : Travel
  BREF : Travel pictures from our {{South Pacific}} crossing and of some of the islands we have seen.
  TYPE : Diary

NIUE
  UNDE : South pacific
  BREF : We have sailed and anchored by the gorgeous coral shores of the country of {{Niue}}.
  TYPE : Diary
  LONG
...
THE SARTRE MECHANISM
  UNDE : Notes
  BREF : It was just three days before Christmas when my friend invited me to his workshop to witness the completion of his latest project.
  LONG
...
PHILIPP TRAUM
  ...
  LONG
    ...
    * NO.44
    ...
    > <mini>Mark Twain's {{the mysterious Stranger|http://en.wikipedia.org/wiki/The_Mysterious_Stranger}}</mini>
    * OBJECTIVE BEINGS
...
THE DAILY DESCENT
  UNDE : Rotonde
  BREF : {{The Daily Descent}} is a daily P2P news blog within {{Rotonde}}.
  LONG
...
MAEVE
  UNDE : Paradise
  BREF : {{Maeve}} is an automated vessel, traveling around the virtual scapes of {{Paradise}} looking for incomplete vessels and errors.
  LONG
...
VERT KIRLIAN THEATRE
  UNDE : First wave
  BREF : The {{Vert Kirlian Theatre}} was created during on a long train ride, on the {{Yamanote line|http://en.wikipedia.org/wiki/Yamanote_Line}}, during my first trip to Tokyo.
  LONG
