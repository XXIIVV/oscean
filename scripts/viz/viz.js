...
function Viz (logs, from, to) {
  ...
  function _legend (logs) {
    ...
    return `
    <text x='${2}' y='${-15}' style='text-anchor:start'>${timeAgo(logs[logs.length - 1].time).toCapitalCase()}</text>
    <text x='${730}' y='${-15}' style='text-anchor:end'>${timeAgo(logs[0].time).toCapitalCase()}</text>
