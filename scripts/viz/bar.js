function BarViz(logs,settings = {size:{width:730,height:125}})
{
  ...
  this.draw = function()
  {
    ...
    html += `<text x='${2}' y='${-10}' style='text-anchor:start'>${this.logs[this.logs.length-1].time.ago().capitalize()}</text>`
    html += `<text x='${730}' y='${-10}' style='text-anchor:end'>${this.logs[0].time.ago().capitalize()}</text>`
