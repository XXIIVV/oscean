function StatusViz(logs,settings = {})
{
  ...
  this.draw = function()
  {
    ...
    <table class='graph status'>
      <tr>
        <th>
          <t class='display'>
            <t class='value'>${now.sum.toFixed(2)}</t>
        ...
        <th>
          <t class='display'>
            <t class='value'>${now.balance.toFixed(2)}</t>
            <t class='offset'>${this.offset(now.balance,before.balance)}</t>
            <t class='unit'>blnc</t>
          </t>
        </th>
        <th>
  ...
  this.style = function()
  {
    return `
    <style>
      #content .graph.status { background:transparent; padding:0px; width:790px}
      #content .graph.status th { font-size:40px; font-family:'archivo_light';line-height:40px; }
      #content .graph.status tr > * { padding:5px 0px}
      #content .graph.status th t.display { display:inline-block; border-bottom:1.5px solid black}
