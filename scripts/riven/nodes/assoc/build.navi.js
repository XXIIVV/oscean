...
RIVEN.lib.Navi = function BuildNaviNode (id, rect) {
  ...
  function _table (portal, term) {
    const _children = portal.children.reduce((acc, child, id) => {
      return `${acc}${`<tr class='head'><th class='${term && child.name === term.name ? 'selected' : ''}'>{(${child.name.toCapitalCase()})}</th><td>${_table_depth1(child, term)}</td></tr>`}`
    }, '')
  ...
  function _table_depth1 (portal, term) {
    const _children = portal.children.reduce((acc, child, id) => {
      return `${acc}${term && child.name === term.name ? `<span class='depth1'><b>${child.name.toCapitalCase()}</b></span> ` : `<span class='depth1'>{(${child.name.toCapitalCase()})}${_table_depth2(child, term)}</span> `}`
    }, '')
  ...
  function _table_depth2 (portal, term) {
    const _children = portal.children.reduce((acc, child, id) => {
      return `${acc}${term && child.name === term.name ? `<span class='depth2'>{*${child.name.toCapitalCase()}*}</span> ` : `<span class='depth2'>{(${child.name.toCapitalCase()})}</span> `}`
    }, '')
    return portal.children.length > 0 ? `(${_children.trim()})` : ''
  }
